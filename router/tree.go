...
func (n *node) addRoute(path string, routeInfo *routeInfo) {
	fullPath := path
	n.priority++

	// Empty tree
	if n.path == "" && n.indices == "" {
		n.insertChild(path, fullPath, routeInfo)
		n.nType = root
		return
	}
...
func (n *node) insertChild(path, fullPath string, routeInfo *routeInfo) {
	for {
...
// given path.
func (n *node) getValue(path string, params *[]string) (routeInfo *routeInfo, tsr bool) {
		...
		if len(path) > len(prefix) {
			if path[:len(prefix)] == prefix {
				...
				case param:
					...
					if routeInfo = n.routeInfo; routeInfo != nil {
						return
					} else if len(n.children) == 1 {
						// No handle found. Check if a handle for this path + a
						// trailing slash exists for TSR recommendation
						n = n.children[0]
						tsr = (n.path == "/" && n.routeInfo != nil) || (n.path == "" && n.indices == "/")
					}
...
walk: // Outer loop for walking the tree
	for len(path) >= npLen && (npLen == 0 || strings.EqualFold(path[1:npLen], n.path[1:])) {
		...
		if len(path) > 0 {
			...
			if !n.wildChild {
				...
				if fixTrailingSlash && path == "/" && n.routeInfo != nil {
					return ciPath
				}
				return nil
			}
			...
			if fixTrailingSlash {
				for i, c := range []byte(n.indices) {
					if c == '/' {
						n = n.children[i]
						if (len(n.path) == 1 && n.routeInfo != nil) ||
							(n.nType == catchAll && n.children[0].routeInfo != nil) {
	...
	if fixTrailingSlash {
		...
		if len(path)+1 == npLen && n.path[len(path)] == '/' &&
			strings.EqualFold(path[1:], n.path[1:len(path)]) && n.routeInfo != nil {
